...
// Sample data to show the list structure
const tasksData = [
    { 
        date: '09/09/2025', 
        tasks: [
...
const TaskForContact = ({ isOpen, onClose, contactName = "Contact Name" }) => {
    ...
    return (
        // Main container (backdrop) jo poori screen ko cover karega
        <div 
            className="fixed inset-0 bg-black/50 bg-opacity-40 flex justify-center items-center z-50 p-4"
            onClick={onClose} // Backdrop par click karne se modal band hoga
        >
            {/* Modal ka content area */}
            <div 
                className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-md transform transition-all flex flex-col"
                onClick={e => e.stopPropagation()} // Content par click karne se modal band nahi hoga
            >
                {/* Header */}
