...
const AppointmentModal = ({ isOpen, onClose, contactId, leadId }: AppointmentModalProps) => {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      getUsers({ createdBy: session?.user?.id }).then(data => {
        setUsers(data);
  ...
  const handleSave = async () => {
    ...
    try {
      await createEvent({
        ...formData,
        eventType: formData.endDate ? 'FROM_TO' : 'START_ONLY',
        contactId,
        leadId,
    ...
    >
      ...
      >
        ...
        <div className="mt-6 space-y-6">
          ...
          <div className="grid grid-cols-[100px_1fr] items-center gap-4">
            <label htmlFor="starts" className="text-sm font-semibold text-gray-900">Starts:</label>
            <input
              type="datetime-local"
              id="starts"
          ...
          <div className="grid grid-cols-[100px_1fr] items-center gap-4">
            <label htmlFor="ends" className="text-sm font-semibold text-gray-900">Ends:</label>
            <input
              type="datetime-local"
              id="ends"
