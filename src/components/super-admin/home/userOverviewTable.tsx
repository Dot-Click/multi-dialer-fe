...
const UserOverviewTable = () => {
    ...
    return (
        <section className="mt-3 bg-[#FFFFFF] dark:bg-slate-800 h-[400px] flex flex-col gap-4 outfit shadow-sm pt-[23px] rounded-[32px] w-full  md:w-[70%]">
            {/* Heading + Buttons */}
            <div className="flex gap-2 justify-between items-center px-[24px] w-[80%]">
                <h1 className="text-[#315189] dark:text-white font-[600] text-[13px] md:text-[18.03px]">
                    Users Overview
                </h1>
  ...
  <table className="min-w-full border-separate" style={{ borderSpacing: "0 8px" }}>
    <thead className="bg-[#FAF9FE] dark:bg-slate-900 sticky top-0">
    ...
    <tbody>
      {users.map((user, index) => (
        <tr
          key={index}
          className="bg-[#FAFAFA] dark:bg-slate-800 rounded-lg hover:bg-gray-100 transition-all"
        >
          <td className="px-4 py-3 text-[#2C2C2C] dark:text-white text-[13.5px] font-[400] whitespace-nowrap">{user.name}</td>
          <td className="px-4 py-3 text-[#2C2C2C] dark:text-white text-[13.5px] font-[400] whitespace-nowrap">{user.email}</td>
          <td className="px-4 py-3 text-[#2C2C2C] dark:text-white text-[13.5px] font-[400] whitespace-nowrap">{user.plan}</td>
          <td className="px-4 py-3">
            <span className={`px-2 py-1.5 rounded-[75px] text-[10.5px] font-[400] ${getStatusClass(user.status)}`}>
              {user.status}
            </span>
          </td>
          <td className="px-4 py-3 text-[#2C2C2C] dark:text-white text-[13.5px] font-[400] whitespace-nowrap">{user.renewal}</td>
          <td className="px-4 py-3 text-[#2C2C2C] dark:text-white text-[13.5px] font-[400] whitespace-nowrap">{user.created}</td>
