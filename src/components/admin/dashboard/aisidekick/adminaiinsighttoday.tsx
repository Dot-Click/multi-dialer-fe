  ...
  return (
    <section className="bg-white dark:bg-slate-800 rounded-xl flex flex-col gap-3 shadow-2xl px-[12px] py-[14px] lg:px-[20px] w-full lg:py-[15px]">
      <div className="flex items-center gap-1 lg:gap-2">
        ...
        />
        <span className="text-[12px] md:text-[17px] lg:text-[20px] font-[500] dark:text-white">
      ...
      <div className="flex items-center justify-between">
        {data.map((dt) => (
          <div key={dt.id} className="flex flex-col">
            <span className="text-[13px] md:text-[24px] lg:text-[40px] font-[600] dark:text-white">
              {dt.number}
            </span>
            <span className="text-[8px] md:text-[10px] lg:text-[14px] text-[#495057] dark:text-gray-400 font-500">
              {dt.name}
            </span>
          </div>
        ))}
