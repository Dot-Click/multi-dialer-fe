...
// import AddCallScoutNumberModal from './AddCallScoutNumberModal'; // Import Modal
...
//             <span
//               className={`px-2.5 py-0.5 text-xs font-medium rounded-full ${isHealthy ? 'bg-green-100 text-green-700' : 'bg-black text-white'
//                 }`}
//             >
//               {data.status}
//             </span>
...
//             className="w-full sm:w-auto bg-yellow-400 text-black font-semibold py-2.5 px-5 rounded-lg flex items-center justify-center gap-2 hover:bg-yellow-500 transition-colors text-sm"
//           >
//             <FiPlus size={18} />
...
const numbersData: NumberData[] = [
  { id: 1, name: 'Number 1', callScoutId: 'CallScout ID', status: 'Healthy', country: 'USA/Canada', availableTo: 'All', addedOn: '09/09/2025' },
  { id: 2, name: 'Number 2', callScoutId: 'CallScout ID', status: 'Healthy', country: 'USA/Canada', availableTo: 'All', addedOn: '09/09/2025' },
];

const NumberCard: React.FC<{ data: NumberData }> = ({ data }) => (
  <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 transition hover:shadow-md">
    <div className="grid grid-cols-1 md:grid-cols-5 gap-4 items-center text-sm">
      <div className="col-span-1">
        <div className="flex items-center gap-2 mb-1">
          <h3 className="font-semibold text-gray-800">{data.name}</h3>
          <span className={`px-2.5 py-0.5 text-xs font-medium rounded-full ${data.status === 'Healthy' ? 'bg-green-100 text-green-700' : 'bg-black text-white'}`}>{data.status}</span>
        </div>
        <p className="text-gray-500 text-xs">{data.callScoutId}</p>
      </div>
      <div className="col-span-1"><p className="text-gray-500 text-xs mb-1">Country</p><p className="font-medium">{data.country}</p></div>
      <div className="col-span-1"><p className="text-gray-500 text-xs mb-1">Available to</p><p className="font-medium">{data.availableTo}</p></div>
      <div className="col-span-1"><p className="text-gray-500 text-xs mb-1">Added on</p><p className="font-medium">{data.addedOn}</p></div>
  ...
  return (
    <div className="bg-[#F9FAFB] min-h-screen px-4 py-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="space-y-4">
          {numbersData.map((number) => <NumberCard key={number.id} data={number} />)}
        </div>
