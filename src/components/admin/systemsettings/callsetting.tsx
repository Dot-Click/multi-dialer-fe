...
const ToggleSwitch: React.FC<ToggleSwitchProps> = ({ enabled, setEnabled }) => (
    <button
        ...
        className={`relative inline-flex h-6 w-11 rounded-full transition-all duration-200 ${enabled ? 'bg-gray-800' : 'bg-gray-300'
            }`}
    >
        <span
            className={`inline-block h-5 w-5 bg-white rounded-full shadow transform transition duration-200 ${enabled ? 'translate-x-5' : 'translate-x-0'
                }`}
        />
    </button>
);
    ...
    return (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 transition hover:shadow-md">
            <div className="hidden lg:grid grid-cols-6 gap-6 items-center text-sm">
                ...
                <div><p className="text-gray-500 text-xs mb-1">Country</p><p className="font-medium text-gray-800">{data.countryCode || 'N/A'}</p></div>
                <div><p className="text-gray-500 text-xs mb-1">Added on</p><p className="font-medium text-gray-800">{new Date(data.createdAt).toLocaleDateString()}</p></div>
                <div><p className="text-gray-500 text-xs mb-1">Auto Pause</p><li className="list-none"><ToggleSwitch enabled={isAutoPause} setEnabled={handleToggleAutoPause} /></li></div>
                <div className="col-span-2 flex justify-end"><button className="text-gray-500 p-2 hover:bg-gray-100 rounded-full"><FiMoreHorizontal size={20} /></button></div>
            </div>
                ...
                <div className="flex justify-between items-start">
                    <div>
                        <div className="flex items-center gap-2 mb-1">
                            ...
                            <span className="px-2.5 py-0.5 text-xs font-medium rounded-full bg-green-100 text-green-700">Healthy</span>
                        </div>
                        <p className="text-gray-500 text-xs">{data.callerId}</p>
                    </div>
                    <button className="text-gray-500 p-2 hover:bg-gray-100 rounded-full"><FiMoreHorizontal size={20} /></button>
                </div>
                <div className="grid grid-cols-2 gap-4">
                    <div><p className="text-gray-500 text-xs mb-1">Country</p><p className="font-medium">{data.countryCode}</p></div>
                    <div><p className="text-gray-500 text-xs mb-1">Added on</p><p className="font-medium">{new Date(data.createdAt).toLocaleDateString()}</p></div>
                    <div><p className="text-gray-500 text-xs mb-1">Auto Pause</p><ToggleSwitch enabled={isAutoPause} setEnabled={handleToggleAutoPause} /></div>
                </div>
            </div>
        </div>
    );
        ...
        <div className="bg-white min-h-screen px-4 py-5 rounded-lg">
            <div className="max-w-7xl mx-auto">
                ...
                <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
                    ...
                    <button className="text-sm font-medium text-gray-600 flex items-center gap-2 p-2 rounded-lg hover:bg-gray-100"><BiSortAlt2 size={18} /> Sort by</button>
                </div>
                <div className="space-y-6">
                    {callSettings?.length === 0 ? (
                        <div className="text-center py-10 border border-dashed border-gray-300 rounded-xl"><p className="text-gray-500">No Call Settings found.</p></div>
                    ) : (
