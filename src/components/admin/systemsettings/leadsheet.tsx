...
const LeadSheet: React.FC = () => {
    const { data: leadSheets, isLoading, isError, error } = useLeadSheets();

    if (isLoading) return <div className="p-8 text-center text-gray-500">Loading Lead Sheets...</div>;
    if (isError) return <div className="p-8 text-center text-red-500">Error: {(error as any)?.message || 'Failed to fetch lead sheets'}</div>;
                ...
                <main className="space-y-4">
                    {leadSheets?.length === 0 ? (
                        <div className="text-center py-10 border border-dashed border-gray-300 rounded-xl">
                            <p className="text-gray-500">No Lead Sheets found. Click "Add Lead Sheet" to create one.</p>
                        </div>
                    ) : (
