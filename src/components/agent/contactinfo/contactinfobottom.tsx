          ...
          {transcriptionLogs.length > 0 ? (
            transcriptionLogs.map((item, index) => (
              <div key={index} className="flex gap-6 items-start">
                {/* Timestamp */}
                <div className="text-gray-400 text-[12px] font-medium w-20 shrink-0 mt-1">
                  {new Date(item.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' })}
                </div>
                
                {/* Message */}
                <div className="flex-1">
                  <p className={`${item.speaker === 'Agent' ? 'text-blue-600' : 'text-green-500'} font-bold text-sm mb-1`}>
                    {item.speaker}
                  </p>
