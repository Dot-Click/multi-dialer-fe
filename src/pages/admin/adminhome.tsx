  ...
  return (
    <section className="w-full min-h-full ">
      ...
      <div className="flex justify-between items-center pr-3 lg:pr-6">
        ...
        {/* Tabs */}
        <div className="flex justify-center px-1.5 py-1.5 rounded-[12px] bg-[#D8DCE1] dark:bg-slate-700 items-center gap-2">
          {components.map((comp) => (
            <button
              key={comp.id}
              ...
              className={`flex items-center gap-2 px-1.5 py-1.5 lg:px-2 lg:py-1 rounded-[8px] transition-all duration-200
                ${active === comp.name ? "bg-white dark:bg-slate-600 shadow-sm" : "bg-transparent"}`}
            >
              <img src={comp.icon} className="h-4 w-4 dark:invert" alt={comp.name} />
              <span
                className={`text-[10px] cursor-pointer md:text-[13px] lg:text-[14px] font-semibold ${
                  active === comp.name
