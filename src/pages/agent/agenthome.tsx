  ...
  return (
    <section className="w-full pb-3 min-h-full">
      ...
      <div className="flex justify-between items-center pr-3 lg:pr-6">
        ...
        {/* Tabs */}
        <div className="flex justify-center px-1.5 py-1.5 rounded-[12px] dark:bg-slate-800 bg-[#D8DCE1] items-center gap-2">
          {components.map((comp) => (
            <button
              key={comp.id}
              ...
              className={`flex items-center gap-2 px-1.5 py-1.5 lg:px-2 lg:py-1 rounded-[8px] transition-all duration-200
                ${active === comp.name ? "bg-white dark:bg-slate-700 shadow-sm" : "bg-transparent"}`}
            >
              <img src={comp.icon} className="h-4 w-4" alt={comp.name} />
              <span
                className={`text-[10px] cursor-pointer md:text-[13px] lg:text-[14px] font-semibold ${active === comp.name ? "text-[#0E1011] dark:text-white" : "text-[#495057]"
                  }`}
              >
                {comp.name}
              </span>
